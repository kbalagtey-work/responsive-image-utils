import { getSrcsetSizes, getMaxNumOfSizes, getImgSizeStrings, getRetinaSizes } from '../src/responsive-image-utils';
...
describe('getSrcsetSizes', () => {
  ...
  it('SHOULD return a PX equivalent for every VW size according to the MQ', () => {
  	expect(getSrcsetSizes({s: '100vw'})).toEqual(expect.arrayContaining(Object.values(MediaQueriesDefault)));
  });

  it('SHOULD return an array containing various sizes of VW', () => {
  	expect(getSrcsetSizes({s: '95vw', l: '45vw'})).toEqual(expect.arrayContaining([ 576, 608, 864, 973, 1152 ]));
  });
...
describe('getImgSizeStrings', () => {
	it(`SHOULD combine the media query and the size provided to create a size string
		...
		expect(getImgSizeStrings({s: '50vw', l: '320px'}, MediaQueriesDefault)).toEqual(expect.arrayContaining(sizeStringArr));
		expect(getImgSizeStrings({s: '50vw', l: '320px'})).toEqual(expect.arrayContaining(sizeStringArr));
	});
});
